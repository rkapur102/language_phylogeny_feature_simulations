# This simulation examines evolutionary dynamics of structured features.
...
feature_val_names <- c("no tones", "simple tone system", "complex tone system") # TONE
...
# to account for the case where there is no specific probability for changing from feature to feature, then set all probabilities to be equal
# feature stability, diffusion, and internal change probabilities are now defined in feature section of simulation code --> they depend on the current feature value

matrix_vals <- c(.7,.15,0,.3,.7,.3,0,.15,.7)
...
quadrilateral <- function( populatedplaces, coor, startingsizesmallq, languages ) { 
...
simulate <- function(languages, largestfamnum, largestlangnum, numtimesteps, chlargeq, degincrease, values, featurestability, diffusionprob, internalchangeprob) { 
  ...
  for(i in 1:numtimesteps) {
    ...
    while( a <= all_langs ) {
      ...
      # FEATURE RELATED DYNAMICS - STRUCTURAL
      ...
      if(pmchoice != "d") {
        ...
        if( option==2 ) { # internal change
          ...
          internalchangeoutcome <- sample(temp_values, size=1, replace=TRUE, prob=rep((1/length(temp_values)),length(temp_values))) # haven't built in complexity yet
